...
\item The wage condition and a joint task extension with a visual check
\item Lean translation: checked theorems, proof steps, and limits
...
\begin{frame}{From tasks to aggregate output}
\[Y=\widetilde B\left(\int_{N-1}^{N}
  y(i)^{(\sigma-1)/\sigma}\,di\right)^{\sigma/(\sigma-1)}.\]
\begin{itemize}
\item The task interval always has measure one.
\item $\sigma>0$ governs substitution across tasks; the $\sigma=1$ case is understood by its limit.
\item Automation changes \emph{who performs} tasks; new task creation shifts the interval to higher $i$.
\end{itemize}
\vspace{.4em}
The resulting aggregate production function has capital and labor weights that depend on technology (Proposition 1).
\end{frame}
...
New complex tasks require labor and replace tasks with lower indices. $W/R$, labor share, employment, and the wage rise.
\end{block}
\end{columns}
\vspace{.7em}
These are Propositions 2 and 3 static comparative statics, not unconditional predictions for a dynamic economy.
\end{frame}

\begin{frame}{Where the wage equation comes from}
\begin{align*}
\text{(B9) }\quad s_L\,d\ln W+(1-s_L)\,d\ln R
  &=d\ln Y|_{K,L},\\
\text{(B10), }dN=0:\quad d\ln W-d\ln R
  &=-\frac{\Lambda_I}{\widehat\sigma+\varepsilon_L}\,dI.
\end{align*}
\vspace{.4em}
Let $x=d\ln W$, $z=d\ln R$, and $p=d\ln Y|_{K,L}$. Since $z=x-(x-z)$, (B9) becomes
\[x=p+(1-s_L)(x-z).\]
Substituting (B10) gives the Proposition 3 wage decomposition. This algebra from two equations is the natural Lean target.
\end{frame}
...
\begin{frame}{Extension: automation and new tasks together}
\begin{columns}[T]
\column{.56\textwidth}
\centering
\begin{tikzpicture}[x=1cm,y=1cm]
\fill[teal!17] (0,0) -- (0,3.5) -- (4.7,3.5) -- cycle;
\fill[orange!18] (0,0) -- (4.7,3.5) -- (4.7,0) -- cycle;
\draw[very thick,navy] (0,0) -- (4.7,3.5);
\draw[->,thick] (0,0) -- (5.05,0) node[below] {$\Delta I$};
\draw[->,thick] (0,0) -- (0,3.85) node[left] {$\Delta N$};
\node[align=center,text=teal,font=\scriptsize\bfseries] at (1.05,3.00)
  {labor task mass\\grows};
\node[align=center,text=orange!75!black,font=\scriptsize\bfseries] at (3.85,1.10)
  {labor task mass\\falls};
\node[fill=white,inner sep=1pt,font=\scriptsize] at (3.45,2.88)
  {$\Delta N=\Delta I$};
\fill[teal] (2.01,2.50) circle (2.5pt);
\node[fill=white,inner sep=1pt,text=teal,font=\scriptsize\bfseries,
  above right=1pt] at (2.01,2.50) {A};
\fill[orange!80!black] (2.01,.80) circle (2.5pt);
\node[fill=white,inner sep=1pt,text=orange!75!black,font=\scriptsize\bfseries,
  above right=1pt] at (2.01,.80) {B};
\end{tikzpicture}
\column{.42\textwidth}
\[\Delta m_L=\Delta N-\Delta I.\]
\vspace{-.3em}
\begin{block}{Marked cases}
\textcolor{teal}{\textbf{A}}\quad $\Delta I=.15$, $\Delta N=.25$\\
\hspace*{1.2em}$\Delta m_L=+.10$\\[.35em]
\textcolor{orange!75!black}{\textbf{B}}\quad $\Delta I=.15$, $\Delta N=.08$\\
\hspace*{1.2em}$\Delta m_L=-.07$
\end{block}
\end{columns}
\vspace{.2em}
\footnotesize Both frontiers must bind and lie inside their task intervals. The plot concerns task mass only; it does not determine wages or employment.
\end{frame}
...
\item At $I^*=I=\widetilde I$, use derivatives from each side.
\end{itemize}
\end{frame}
...
\column{.40\textwidth}
\centering
\IfFileExists{hand/derivation.jpg}{\includegraphics[height=.72\textheight]{hand/derivation.jpg}}{
\fbox{\parbox{.92\linewidth}{\centering Real photograph of the student's handwritten derivation pending. See \texttt{hand/README.md}.}}}
\column{.57\textwidth}
\begin{block}{What equation (B10) fixes}
\[
\frac{d\ln(W/R)}{dI}
=-\frac{\Lambda_I}{\widehat\sigma+\varepsilon_L}<0.
\]
The wage falls relative to the rental price of capital.
\end{block}
\begin{alertblock}{My verdict after solving (B9)--(B10)}
\[
\frac{d\ln W}{dI}=P_I-D_I,
\qquad
W\uparrow\iff P_I>D_I.
\]
A fall in $W/R$ does not imply a fall in $W$: productivity can dominate displacement.
\end{alertblock}
\end{columns}
\end{frame}

\input{lean-slides.tex}
...
\item Matching the NBER claim requires a separate comparison between the source and Lean statement.
